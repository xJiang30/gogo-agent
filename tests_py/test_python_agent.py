...
class FakeProvider:
    ...
    def generate_local_proposal(self, *, system_prompt, user_prompt):
        payload = json.loads(user_prompt)
        node = payload.get("current_node", {})
        return {
            "title": f"Replace {node.get('title', 'selected node')}",
            "summary": "Use a concrete model-generated replacement for this node.",
            "reasons": ["The requested change can be proposed locally."],
            "warnings": [],
            "replacement": {
                "title": f"Model replacement for {node.get('title', 'selected node')}",
                "detail": "Concrete replacement generated by the test provider.",
                "location": node.get("location"),
            },
            "evidence": [{"id": "local-evidence-1", "kind": "planner_note", "text": "Generated by local proposal provider."}],
        }
...
class LocalProposalProvider(FakeProvider):
    def __init__(self):
        self.local_proposal_calls = []
        self.answer_calls = []
    ...
    def generate_local_proposal(self, *, system_prompt, user_prompt):
        self.local_proposal_calls.append({"system_prompt": system_prompt, "user_prompt": user_prompt})
        return {
            "title": "Replace hotel with a quieter stay",
            "summary": "Use a concrete quieter hotel candidate near the same area.",
            "reasons": ["The user requested a quieter hotel."],
            "warnings": ["Confirm availability before booking."],
            "replacement": {
                "title": "Quiet riverside hotel near the original area",
                "detail": "A calmer stay option with easier rest and similar access to planned activities.",
                "location": "same neighborhood",
            },
            "evidence": [{"id": "local-evidence-1", "kind": "planner_note", "text": "Generated by local proposal model."}],
        }


class FailingLocalProposalProvider(FakeProvider):
    def generate_local_proposal(self, *, system_prompt, user_prompt):
        raise LlmProviderError(code="timeout_error", message="local proposal timeout", retryable=True)


class IncompleteLocalProposalProvider(FakeProvider):
    def generate_local_proposal(self, *, system_prompt, user_prompt):
        return {
            "replacement": {
                "title": "Quiet riverside hotel",
                "detail": "A quieter stay near the original area.",
            },
        }
...
class PythonAgentTest(unittest.TestCase):
    ...
    def test_free_text_replace_is_consultative_and_does_not_call_local_proposal_provider(self):
        provider = LocalProposalProvider()
        advisor = TravelAdvisor(graph=TripPlanningGraph(provider=provider))
        trip = {
            "id": "trip-1",
            "version": 1,
        ...
        self.assertEqual(len(provider.local_proposal_calls), 0)
        self.assertEqual(len(provider.answer_calls), 1)

    def test_local_proposal_provider_failure_returns_fallback_instead_of_template_proposal(self):
        advisor = TravelAdvisor(graph=TripPlanningGraph(provider=FailingLocalProposalProvider()))
        trip = {
            "id": "trip-1",
            "version": 1,
            "days": [{"id": "day-1", "nodes": [{"id": "node-1", "type": "hotel", "title": "Noisy Hotel"}]}],
    ...
    def test_local_proposal_requires_provider_title_and_summary(self):
        advisor = TravelAdvisor(graph=TripPlanningGraph(provider=IncompleteLocalProposalProvider()))
        trip = {
            "id": "trip-1",
            "version": 1,
            "days": [{"id": "day-1", "nodes": [{"id": "node-1", "type": "hotel", "title": "Noisy Hotel"}]}],
        ...
        result = advisor.handle_request(
            user_id="user-1",
            conversation_id="conversation-1",
            user_message="Please replace this hotel with something quieter",
            trip=trip,
            intent={"kind": "replace_hotel", "node_id": "node-1", "affected_day_ids": ["day-1"]},
        )
    ...
    def test_actual_fallback_shapes_match_contract(self):
        ...
        local = AdvisorFallbackDecision.model_validate(
            {
                "layer": "advisor",
                "decision": "missing_node_for_local_proposal",
        ...
        self.assertEqual(local.missing, ["node_id"])
    ...
    def test_qwen_provider_new_structured_methods_use_same_config_guard(self):
        ...
        for method_name in ["generate_replan_routing_decision", "generate_specialist_selection", "generate_local_proposal"]:
